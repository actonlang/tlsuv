...
int p11_load_rsa(mbedtls_pk_context *pk, struct mp11_key_ctx_s *p11key, mp11_context *p11) {
    ...
    pubattr[0].pValue = tlsuv_malloc(pubattr[0].ulValueLen);
    pubattr[1].pValue = tlsuv_malloc(pubattr[1].ulValueLen);
    rc = p11->funcs->C_GetAttributeValue(p11->session, p11key->pub_handle, pubattr, 2);
    if (rc != CKR_OK) {
        return MBEDTLS_ERR_PK_KEY_INVALID_FORMAT;
    }

    mbedtls_rsa_context *rsa = tlsuv_malloc(sizeof(mbedtls_rsa_context));
...
static int p11_rsa_sign(void *ctx, mbedtls_md_type_t md_alg,
    ...
    CK_BYTE *msg = tlsuv_malloc(hash_len + oid_len);
